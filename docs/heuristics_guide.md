...
- Two disjoint patterns: e.g., tiles {1,2,3,4} and {5,6,7,8} (blank ignored or included in one).  
- Or one 5-tile + one 4-tile pattern.  
- Precompute via BFS from goal substates → store in a dictionary or array (abstract state → exact distance).

**How to Abstract a State for Lookup**  
- Map only the positions of tiles in the pattern to a single integer key (e.g., permutation index or flattened tuple).  
- Ignore other tiles → many real states map to the same abstract state.

**Example Workflow**  
1. At startup: Run BFS for each pattern from goal → build lookup table (e.g., 9!/5! ≈ 15k entries for 5-tile — tiny).  
2. For a state s:  
   - For pattern 1: extract positions of its tiles → lookup exact cost c1.  
   - For pattern 2: same → c2.  
   - h(s) = c1 + c2 (additive).  
   - Often combined with Manhattan fallback for uncovered cases.

**Properties**  
- Admissible: Yes (exact cost for subset is a lower bound on full cost).  
- Consistent: Usually yes (if patterns are properly chosen).  
- Computation: Lookup O(1) after precompute; precompute is fast for 8-puzzle.  
...
|------------------------|----------------|----------|------------------------|
| Misplaced Tiles        | 143,840        | 0.546    | —                      |
| Manhattan              | 20,291         | 0.119    | baseline               |
| Linear Conflict        | 12,293         | 0.174    | 39% fewer nodes        |
...
| Heuristic              | Nodes Expanded | Time (s) | Reduction vs Manhattan |
|------------------------|----------------|----------|------------------------|
| Misplaced Tiles        | 1,643          | 0.006    | —                      |
| Manhattan              | 204            | 0.001    | baseline               |
| Linear Conflict        | 109            | 0.002    | 47% fewer nodes        |
