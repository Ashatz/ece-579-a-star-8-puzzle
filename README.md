...
```bash
# Clone the repo
git clone https://github.com/Ashatz/ece-579-a-star-8-puzzle.git
...
# Misplaced tiles
python puzzle_cli.py puzzle solve "1,2,3,4,5,6,7,*,8" --heuristic misplaced

# Linear conflict
python puzzle_cli.py puzzle solve "8,6,7,2,5,4,3,*,1" --heuristic linear-conflict

# Pattern database
python puzzle_cli.py puzzle solve "6,1,8,4,*,2,7,3,5" --heuristic pattern-db

# Custom goal
python puzzle_cli.py puzzle solve "2,8,3,1,6,4,7,*,5" \
  --goal "1,2,3,8,*,4,7,6,5" --heuristic manhattan

# Different blank symbol
python puzzle_cli.py puzzle solve "2 8 3 1 6 4 7 * 5" --goal "1,2,3,8,*,4,7,6,5" --blank-symbol "_"
```

See `python puzzle_cli.py puzzle solve --help` for full list of supported heuristics and options.
...
Runs a predefined suite (trivial → hard puzzles + one unsolvable case) with all supported heuristics, printing side-by-side metrics: nodes expanded, execution time, and path length.
